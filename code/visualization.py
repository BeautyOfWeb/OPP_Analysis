...
def plot_curves(xs, colors, labels, figsize=(20, 10), show=True, **kwargs):
    ...
    Args:
        xs: list of 1-d numpy array or 2-d numpy array
...
def imshow(tensor, dim=None, title='', figsize=(20, 10), show_colorbar=True, show=True, save_file=None, display=True, **kwargs):
    """Wrapper of plt.imshow() that can directly show 2D torch.Tensor as an image
    """
...
def plot_tensor(x, y=None, marker='o', figsize=None, title=None, show=False, **kwargs):
    """Wrapper of plt.plot that can directly plot 1D torch.Tensor
    """
    if isinstance(x, torch.Tensor):
...
def plot_singular_values(mat, marker='o', figsize=None, title=None, show=True, return_svd=False, use_cpu=False, end=None, **kwargs):
    """Call torch.svd() and plot singular values
    Args:
        mat: 2D torch.Tensor; if mat.ndim>2, mat = mat.reshape(mat.size(0), -1)
    """
...
def plot_multiple_traces(traces, colors, multiple_figures=False, figsize=(20, 20)):
    """Plot multiple traces
    
    Args:
        traces: torch.Tensor or list of torch.Tensor or numpy array
    """
    if isinstance(traces, torch.Tensor):
        traces = traces.cpu()
    if isinstance(traces, list) and len(traces)>0 and isinstance(traces[0], torch.Tensor):
        traces = [trace.cpu() for trace in traces]
    if not multiple_figures:
        fig, ax = plt.subplots(figsize=figsize)
    for i in range(len(traces)):
        if multiple_figures:
            fig, ax = plt.subplots(figsize=figsize)
        ax.plot(traces[i], label=i+1, c=colors[i%len(colors)], alpha=1)
        ax.text(-len(traces[i])*0.02, traces[i, :10].mean(), i+1, c=colors[i%len(colors)])
        if multiple_figures:
            plt.legend()
            plt.show()
    if not multiple_figures:
...
def plot_traces(traces, colors, image=None, label_image=None, figsize=(20, 20)):
    """Plot multiple traces
    """
    if image is not None and label_image is not None:
    ...
    plot_multiple_traces(traces, colors, multiple_figures=False, figsize=figsize)
    # plot each trace separately
    plot_multiple_traces(traces, colors, multiple_figures=True, figsize=figsize)
...
def plot_3d_scatter(img, filter_fn=None, s=1, alpha=.1, cmap=plt.get_cmap('cool'), title=None, figsize=None, labels=None, 
                   set_lim=False, show=True, set_equal_axis=False):
    """3D scatter plot
...
def get_good_colors(num=None, max_num=100, min_dist=0.15, min_dist_white=0.3, 
    ...
    """Generate a list of most distinguishable colors
    """
    if seed is None:
...
def plot_colortable(colors, title='Selected Colors', sort_colors=False, cell_width=300, emptycols=0):
    """Plot a list of colors in a table
    Args:
...
def save_gif_file(imgs, save_path, duration_per_frame=1500):
    ...
    Args:
        imgs: file paths for a list of images
        save_path: gif file path
        duration_per_frame: default 1500 (1.5s) per frame
    """
