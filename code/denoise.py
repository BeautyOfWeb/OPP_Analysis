...
class SeparateNet(nn.Module):
    """Reimplement Mehrtash's CellMincer denoising model (with a 2D spatial network and a 1D temporal network)
...
def get_noise2self_train_data(mat, ndim:int=3, batch_size:int=5, frame_depth:int=6, mask_prob:float=0.05, 
                              features=None, frame_weight=None, frame_indices=None, 
                              return_frame_indices=False, window_size_row=None, window_size_col=None, weight=None):
    """Prepare a batch of training data for Noise2Self
    
    Args:
        mat: torch.Tensor with shape (nframe, nrow, ncol); 
            #Todo: it should work for 4D tensor with shape (nframe, nchannels, nrow, ncol), but is not fully implemented or tested
        ndim: int, default 3, prepare for nn.Conv3D input; if ndim==2, then prepare for nn.Conv2D input
        batch_size: int, the number of frames to be predicted in a batch
        frame_depth: 2*frame_depth+1 is number of frames used to denoise the middle frame
        mask_prob: float in (0, 1), the probability of a pixel being masked by a random number
        features: default None; if not None, must be a torch.Tensor with shape (num_features, nrow, ncol)
        frame_weight: default None; if not None, len(frame_weight) == nframe-2*frame_depth+1; only used when frame_indices is None
        frame_indices: default None; if not None, it is a list of frame indices to be predicted
        return_frame_indices: if True, return selected frame indices for prediction
        window_size_row: default None; if not None, it is an int, window_size_row < nrow
        window_size_col: default None; if not None, it is an int, window_size_col < ncol
        weight: default None; if not None, it should be a torch.Tensor specifying the weight for loss function such as nn.MSELoss()
        
    Returns:
        output_dict: a dictionary with keys:
            x: masked_input of shape (batch_size, [1,] 2*frame_depth+1, nrow, ncol)
            y_true: frames to be predicted (batch_size, nrow, ncol)
            frame_indices: if return_frame_indices is True
    ...
    x[:, frame_depth] = x[:, frame_depth] * (1-mask) + mask * (torch.randn(batch_size, nrow, ncol, dtype=mat.dtype, device=mat.device)*std + mean)
    if ndim==3:
        x = x.unsqueeze(1)
...
def model_denoise(mat, model, batch_size:int=20, frame_depth:int=6, normalize=True, features=None, ndim:int=3, frame_indices=None, replicate_pad=True):
    """Denoise mat with model
    
    Args:
        mat: torch.Tensor of shape (nframe, nrow, ncol)
        model: nn.Module
        normalize: default True, making sure mat.mean() == 0 and mat.std() == 1 for model input
        features: default None; if not None, they are "global features" to be feeded to model along with frames
        frame_indices: default None, predict the middle nframe-2*frame_depth+1 frames; otherwise predict only frames with indices among frame_indices
        replicate_pad: default True, replicate pad the output with frame_depth frames in the beginning and the end
    
    Returns:
        y: torch.Tensor, denoised version of mat
...
def get_denoised_mat(mat, model=None, out_channels=[64, 64, 64], ndim=3, num_epochs=12, num_iters=600, print_every=300, batch_size=2,
                     ...
                     features=None, fps=60, save_filename='denoised_mat',
                     window_size_row=None, window_size_col=None, weight=None, verbose=False, return_model=False, 
                     half_precision=False, device=torch.device('cuda')):
...
#             if epoch == num_epochs-1:
    ...
    if save_filename is None:
        ...
        np.save(f'{save_folder}/{save_filename}.npy', denoised_mat.cpu().numpy())
    if verbose:
        end_time = time.time()
