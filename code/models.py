...
class DenseNet(nn.Module):
    """Implement MLP, DenseNet and ResNet with nn.Linear layers

    Args:
        in_dim: the last dimension of input
        hidden_dims: a list or tuple of dimensions in the hidden layers, including the output dimension at the end
        dense: if True, implement DenseNet
        residual: if True, implement ResNet
            dense and residual cannot be True at the same time
            If both dense and residual are False, then it is a simple MLP

    """
    def __init__(self, in_dim:int, hidden_dims, dense=False, residual=False, nonlinearity=nn.LeakyReLU()):
...
class DownConv(nn.Module):
    r"""Apply MaxPool to reduce size followed by a MultiConv layer
    Args:
        pool_kernel_size: int
        Most arguments are the same as MultiConv except pool_kernel_size

    """
    def __init__(self, in_channels:int, out_channels:int, num_conv:int=2, n_dim:int=2, kernel_size:int=3, padding:int=1, 
        padding_mode='replicate', same_shape=True, pool_kernel_size:int=2, normalization='layer_norm', 
...
class UpConv(nn.Module):
    r"""Apply nn.ConvTranpose to increase size followed by MultiConv
    Args:
        transpose_kernel_size: int, default 2
        transpose_stride: int, default 2
        use_adaptive_pooling: if True, use nn.AdaptivePool instead of padding to ensure the output has a proper size
        Most other arguments are used in MultiConv

    """
    def __init__(self, in_channels:int, out_channels:int, num_conv:int=2, n_dim:int=2, kernel_size:int=3, padding:int=1, 
        padding_mode='replicate', same_shape=True, normalization='layer_norm', use_adaptive_pooling=False, 
        transpose_kernel_size:int=2, transpose_stride=2, activation=nn.LeakyReLU(negative_slope=0.01, inplace=True)):
...
def restore_image_noise2self(model, x, bg_masks=None, kernel_size=3, bg_mat=None, is_train=False):
    """Restore image from noise2self
    Args:
        model: nn.Module
        x: torch.Tensor
        bg_masks: default None, construct internally
        bg_mat: default None, replace masked elements with 0; if not None, replace masked elements with elements in bg_mat
        is_train: default False, disable autograd to speed up

    Returns:
        y_pred: torch.Tensor (denoised version of x)
